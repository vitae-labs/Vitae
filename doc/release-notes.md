...
Vitae Core version 0.20.0 is now available from:

  <https://vitaecore.org/bin/vitae-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Vitae
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Vitae Core on
unsupported systems.

From Vitae Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Vitae Core does not yet change appearance
...
#### Removal of BIP61 reject network messages from Vitae Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Vitae Core version 0.18.0.
...
- Testing or debugging of implementations of the Vitae P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Vitae Core. Vitae Core logs debug messages
...
- OpenSSL is no longer used by Vitae Core.  (#17265)

- BIP70 support has been fully removed from Vitae Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/vitae/vitae/issues/16599),
  [PR #16702](https://github.com/vitae/vitae/pull/16702), and the
  `vitaed help` for more information.  This option is experimental and
...
- All custom settings configured when Vitae Core starts are now
...
- The "Start Vitae Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://vitaecore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Vitae Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Vitae Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/vitae-core/HWI) for
  signing.  Future versions of Vitae Core should support a GUI option
...
- Vitae Core's automatically-generated source code documentation is
  now available at https://doxygen.vitaecore.org.  (#17596)
...
- The `vitae-cli` utility used with the `-getinfo` parameter now
...
- The `vitae-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `vitaestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_vitae-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBVITAE_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_vitae` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBVITAE_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for vitae-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate vitae schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest vitae.conf (sdaftuar)
...
- #18022 Fix appveyor `test_vitae` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (vitae-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone vitae-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Vitae Core files (hebasto)
...
- #17119 Fix broken vitae-cli examples (andrewtoth)
...
- #17503 Remove bitness from vitae-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.vitaecore.org (laanwj)
...
- #17648 Rename wallet-tool references to vitae-wallet (hel-o)
...
- #18341 Replace remaining literal VITAE with `CURRENCY_UNIT` (domob1812)
...
- #16115 On vitaed startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from vitae-cli -getinfo (malevolent)
...
[Transifex](https://www.transifex.com/vitae/vitae/).
