...
#ifndef VITAE_WALLET_WALLET_H
#define VITAE_WALLET_WALLET_H
...
extern bool fUnlockForStakingOnly;
...
public:
     ...
     * on this vitae node, and set to 0 for transactions that were created
...
public:
    ...
    bool Unlock(const SecureString& strWalletPassphrase, bool accept_no_keys = false, bool stakeOnly = false);
    ...
    //! staking related wallet function call/variables
    unsigned int nHashDrift = 45;
    unsigned int nHashInterval = 22;
    int nStakeSetUpdateTime = 300;
    int nStakeSplitThreshold = 2000;

    using StakeCoinsSet = std::set<std::pair<const CWalletTx*, unsigned int> >;

    bool MintableCoins() const;
    bool SelectStakeCoins(StakeCoinsSet &setCoins, CAmount nTargetAmount, bool fSelectWitness, const CScript &scriptFilterPubKey) const;
    bool CreateCoinStake(/*const SigningProvider& keystore,*/ unsigned int nBits, CAmount blockReward, CMutableTransaction &txNew, unsigned int &nTxNewTime/*, std::vector<const CWalletTx*> &vwtxPrev*/, bool fGenerateSegwit, int &nHeight) const;
