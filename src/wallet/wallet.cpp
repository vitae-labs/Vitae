...
bool CWallet::Unlock(const SecureString& strWalletPassphrase, bool accept_no_keys, bool stakeOnly)
{
    if (!IsLocked()) {
        fWalletUnlockStakeOnly = stakeOnly;
        return true;
    }
    CCrypter crypter;
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose)
{
    ...
    if (!strCmd.empty())
    {
        ...
        // https://github.com/vitae/vitae/pull/13339#issuecomment-537384875
        // A few ways it could be implemented in the future are described in:
        // https://github.com/vitae/vitae/pull/13339#issuecomment-461288094
...
{
    // If wallet doesn't have a chain (e.g vitae-wallet), lock can't be taken.
    ...
    while (block_height && !fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (chain().findBlock(block_hash, &block) && !block.IsNull()) {
            ...
            if (!locked_chain->getBlockHeight(block_hash)) {
                ...
                // https://github.com/vitae/vitae/pull/14711#issuecomment-458342518
...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl* coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount, bool isSearchingMasternode) const
{
    AssertLockHeld(cs_wallet);
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if (IsLockedCoin(entry.first, i) && !isSearchingMasternode)
                continue;
...
DBErrors CWallet::LoadWallet(bool& fFirstRunRet)
{
    ...
    // tx status. If lock can't be taken (e.g vitae-wallet), tx confirmation
 ...
 * https://vitaetalk.org/?topic=54527, or
 * https://github.com/vitae/vitae/pull/1393.
 */
...
bool CWallet::SelectStakeCoins(StakeCoinsSet &setCoins, CAmount nTargetAmount, bool fSelectWitness, const CScript &scriptFilterPubKey) const
{
    ...
    for (const COutput& out : vCoins) {
            ...
            continue;
        {
            ...
            txnouttype which_type = Solver(scriptPubKeyKernel, vSolutions);
            if ( which_type != txnouttype::TX_PUBKEY && which_type != TX_PUBKEYHASH) {
                continue;
            }
        }

        if (fMasterNode && out.tx->tx->vout[out.i].nValue == 20000 * COIN)
            continue;
        ...
        if (out.nDepth < (out.tx->tx->IsCoinStake() ? Params().GetConsensus().nMinStakeHistory : Params().GetConsensus().nMinStakeHistory))
            continue;
...
bool CWallet::CreateCoinStakeKernel(CScript &kernelScript, const CScript &stakeScript, unsigned int nBits, const CBlock &blockFrom, unsigned int nTxPrevOffset, const CTransactionRef &txPrev, const COutPoint &prevout, unsigned int &nTimeTx, bool fPrintProofOfStake) const
{
    ...
    {
        nTryTime = nTimeTx - i;
        if (CheckStakeKernelHash(::ChainActive().Tip(), nBits, blockFrom, nTxPrevOffset, txPrev, prevout, nTryTime, hashProofOfStake, true, false))
        {
...
typedef std::vector<unsigned char> valtype;
bool CWallet::CreateCoinStake(/*const SigningProvider& keystore,*/ unsigned int nBits, CAmount blockReward, CMutableTransaction &txNew, unsigned int &nTxNewTime/*, std::vector<const CWalletTx*> &vwtxPrev*/, bool fGenerateSegwit, int &nHeight) const
{
