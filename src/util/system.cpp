...
int64_t enforceMasternodePaymentsTime = 1641037796;//4085657524;
bool fForceStaking = false;
StakingStatus stakestat = DISABLED;
...
const char * const VITAE_CONF_FILENAME = "vitae.conf";
 ...
 * https://github.com/vitae/vitae/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //vitae-tx using stdin
        std::string val;
    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::MASTERNODES:
                usage += HelpMessageGroup("Masternodes Commands:");
                break;
            default:
                    break;
        }
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VITAE_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Vitae Core copyright is not removed by accident
//    if (copyright_devs.find("Vitae Core") == std::string::npos) {
