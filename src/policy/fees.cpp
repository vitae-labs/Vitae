...
void CBlockPolicyEstimator::processTransaction(const CTxMemPoolEntry& entry, bool validFeeEstimate)
{
    ...
    // Feerates are stored and reported as VITAE-per-kb:
...
{
    LOCK(m_cs_fee_estimator);
    CFeeRate defaut_fee = CFeeRate(100000);
    ...
    if (confTarget <= 0 || (unsigned int)confTarget > longStats->GetMaxConfirms()) {
        return defaut_fee;  // error condition
    }
    ...
    if (confTarget <= 1) return defaut_fee; // error condition
    ...
    if (median < 0) return defaut_fee; // error condition
