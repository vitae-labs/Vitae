...
public:
    ...
    // Masternode messages time:
    // Time (in usec) the last mnget was sent, or 0 if never sent
    std::atomic<int64_t> nLastMnGetUsec{0};
    // Time (in usec) the last mnget was sent, or 0 if never sent
    std::atomic<int64_t> nLastGetSporksUsec{0};
    // Requested my RPC (future implimentation)
    std::atomic<bool> fRPCmnrequest{false};
    // Is mn synced?
    std::atomic<bool> fmnsynced{false};
