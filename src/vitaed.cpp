...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  vitaed [options]                     Start " PACKAGE_NAME "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(strprintf("Command line contains unexpected token '%s', see vitaed -h for a list of options.\n", argv[i]));
            }
        }

        // -server defaults to true for vitaed but not for the GUI so do this here
