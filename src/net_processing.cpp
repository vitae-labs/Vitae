...
void static ProcessGetData(CNode* pfrom, const CChainParams& chainparams, CConnman* connman, const CTxMemPool& mempool, const std::atomic<bool>& interruptMsgProc) LOCKS_EXCLUDED(cs_main)
{
...

    {
        ...
        while (it != pfrom->vRecvGetData.end() && (it->type == MSG_TX ||
                                                   ...
                                                   it->type == MSG_MASTERNODE_SCANNING_ERROR ||
                                                   it->type == MSG_MASTERNODE_WINNER ||
                                                   it->type == MSG_SPORK)) {
            if (interruptMsgProc)
                return;
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && orphan_state.GetResult() != TxValidationResult::TX_WITNESS_MUTATED) {
                ...
                // See https://github.com/vitae/vitae/issues/8279 for details.
                assert(recentRejects);
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::ADDR) {
        ...
        vRecv >> vAddr;
        //LogPrintf("NetMsgType::ADDR: received : doing nothing\n");
        //return true;// todo:
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        std::vector<CInv> vBlocksToFetch;
        ...
        for (CInv &inv : vInv)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if((pfrom->nVersion < PROTOCOL_VERSION + 2)){
                        //std::cout<<" proto pass \n";
                        vBlocksToFetch.push_back(inv);
                    }
        ...
        if (!vBlocksToFetch.empty()) {
            connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::GETDATA, vBlocksToFetch));
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        std::vector<CBlockHeader> headers; headers.clear();
    ...
    // If nothing returned till now then check if that was some mn message
    mnodeman.ProcessMessage(pfrom, msg_type, vRecv, connman);
    ProcessMessageMasternodePayments(pfrom, msg_type, vRecv, connman);
    ProcessSpork(pfrom, msg_type, vRecv, connman);
    ProcessMessageMasternodePOS(pfrom, msg_type, vRecv, connman);
    return true;
}
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // masternode messages
        //
        bool mngetSend = false;
        bool getsporksSend = false;
        // requested by rpc
        if (pto->fRPCmnrequest) {
            mngetSend = true;
            getsporksSend =true;
        }
        if (pto->nLastMnGetUsec == 0 && pto->nLastMnGetUsec + 1*60/* one minute*/ * 1000000 < GetTimeMicros()) {
            // Ping automatically sent as a latency probe & keepalive.
            mngetSend = true;
        }
        if (pto->nLastGetSporksUsec == 0 && pto->nLastGetSporksUsec + 1*60/* one minute*/ * 1000000 < GetTimeMicros()) {
            // Ping automatically sent as a latency probe & keepalive.
            getsporksSend = true;
        }
        if (mngetSend) {
            if (!::ChainstateActive().IsInitialBlockDownload() && !pto->fmnsynced) {
                ...
                connman->PushMessage(pto, msgMaker.Make("getsporks"));
                pto->nLastMnGetUsec = GetTimeMicros();
                pto->nLastGetSporksUsec = GetTimeMicros();
            }
        }
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - /*24 * 60 **/ 60) {
                ...
                /* If possible, start at the block preceding the currently
                   ...
                   
        ...
        while (!tx_process_time.empty() && tx_process_time.begin()->first <= current_time && state.m_tx_download.m_tx_in_flight.size() < MAX_PEER_TX_IN_FLIGHT) {
            CInv inv = tx_process_time.begin()->second;
            // Erase this entry from tx_process_time (it may be added back for
            // processing at a later time, see below)
            tx_process_time.erase(tx_process_time.begin());
            if (inv.type == MSG_TX) {
                CInv inv_(MSG_TX | GetFetchFlags(pto), inv.hash);
                inv = inv_;
            }
