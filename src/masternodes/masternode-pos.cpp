...
void CMasternodeScanning::DoMasternodePOSChecks()
{
    ...
    // -- first check : Port is open
        ...
        bool ConnectNodeCheck;
        CAddress addr(pmn->addr, NODE_NONE);
        g_mn_node->connman->OpenNetworkConnection(addr, false, NULL, NULL);
...
bool CMasternodeScanningError::SignatureValid()
{
    ...
    CScript pubkey;
    pubkey = GetScriptForDestination(PKHash(pmn->pubkey2));
...
bool CMasternodeScanningError::Sign()
{
    ...
    CScript pubkey;
    pubkey = GetScriptForDestination(PKHash(pubkey2));
    CTxDestination address1;
    ExtractDestination(pubkey, address1);
    //CBitsendAddress address2(address1);
