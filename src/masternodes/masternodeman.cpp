...
void CMasternodeMan::DsegUpdate(CNode* pnode, CConnman *pconnman)
{
    LOCK(cs);
    ...
    //pnode->PushMessage("dseg", CTxIn());
        pconnman->PushMessage(pnode, CNetMsgMaker(PROTOCOL_VERSION).Make(SERIALIZE_TRANSACTION_NO_WITNESS, "dseg", CTxIn()));
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman* connman)
{
    ...
    if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        strMessage = addr.ToString() + boost::lexical_cast<std::string>(sigTime) + vchPubKey + vchPubKey2 + boost::lexical_cast<std::string>(protocolVersion)  + HexStr(donationAddress) + boost::lexical_cast<std::string>(donationPercentage);
        ...
        uint256 blockhash_;
        if(!darkSendSigner.IsVinAssociatedWithPubkey(vin, pubkey, blockhash_)) {
        ...
        if(AcceptableInputs(mempool, state, MakeTransactionRef(mtx))){
            ...
            BlockMap::iterator mi = ::BlockIndex().find(blockhash_);
            ...
            //vAddr = CAddress(addr);
            CAddress addr_(addr, NODE_NONE);
            vAddr.push_back(addr_);
            ...
            if((GetAdjustedTime() - pmn->lastVote) > (60*60))
            {
                ...
                BOOST_FOREACH(CNode* pnode, vNodes)
                    pnode->PushMessage("mvote", vin, vchSig, nVote);*/
                                connman->ForEachNode([ &vin, &vchSig, &nVote, connman](CNode* pnode)
				{
...
void CMasternodeMan::RelayMasternodeEntry(const CTxIn vin, const CService addr, const std::vector<unsigned char> vchSig, const int64_t nNow, const CPubKey pubkey, const CPubKey pubkey2, const int count, const int current, const int64_t lastUpdated, const int protocolVersion, CScript donationAddress, int donationPercentage, CConnman* connman)
{
	...
	*/
        connman->ForEachNode([ &vin, &addr, &vchSig, &nNow, &pubkey, &pubkey2, &count, &current, &lastUpdated, &protocolVersion, &donationAddress, &donationPercentage, connman](CNode* pnode)
    {
...
void CMasternodeMan::RelayMasternodeEntryPing(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop, CConnman* connman)
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodes)
        ...
        connman->ForEachNode([ &vin, &vchSig, &nNow, &stop, connman](CNode* pnode)
    {
...
void CMasternodeMan::RelayNormalMasternodeEntry(const CTxIn vin, const CService addr, const std::vector<unsigned char> vchSig, const int64_t nNow, const CPubKey pubkey, const CPubKey pubkey2, const int count, const int current, const int64_t lastUpdated, const int protocolVersion, CScript donationAddress, int donationPercentage)
{
    /*LOCK(cs_vNodes);
     ...
     pnode->PushMessage("dsee", vin, addr, vchSig, nNow, pubkey, pubkey2, count, current, lastUpdated, protocolVersion, donationAddress, donationPercentage);
        */
    g_mn_node->connman->ForEachNode([&vin, &addr, &vchSig, &nNow, &pubkey, &pubkey2, &count, &current, &lastUpdated, &protocolVersion, &donationAddress, &donationPercentage](CNode* pnode)
    {
        g_mn_node->connman->PushMessage(pnode, CNetMsgMaker(PROTOCOL_VERSION).Make(SERIALIZE_TRANSACTION_NO_WITNESS, "dsee", vin, addr, vchSig, nNow, pubkey, pubkey2, count, current, lastUpdated, protocolVersion, *(CScriptBase*)(&donationAddress), donationPercentage));
    });
}
...
void CMasternodeMan::RelayNormalMasternodeEntryPing(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop)
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodes)
        pnode->PushMessage("dseep", vin, vchSig, nNow, stop);*/
    g_mn_node->connman->ForEachNode([&vin, &vchSig, &nNow, &stop](CNode* pnode)
    {
        g_mn_node->connman->PushMessage(pnode, CNetMsgMaker(PROTOCOL_VERSION).Make(SERIALIZE_TRANSACTION_NO_WITNESS, "dseep", vin, vchSig, nNow, stop));
    });
}
