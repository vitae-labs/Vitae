...
{	
    ...
    bool fIsInitialDownload = ::ChainstateActive().IsInitialBlockDownload();
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string txHash, std::string strOutputIndex, std::string strDonationAddress, std::string strDonationPercentage, std::string& errorMessage) {
    ...
    // maturity check
    if(GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS) {
        errorMessage = "not enough confirmations need atleast 15 to start masternode.";
        LogPrintf("CActiveMasternode::Register() - Error: %s\n", errorMessage.c_str());
        return false;
    }
...
bool CActiveMasternode::Register(CTxIn vin, CService service, CKey keyCollateralAddress, CPubKey pubKeyCollateralAddress, CKey keyMasternode, CPubKey pubKeyMasternode, CScript donationAddress, int donationPercentage, std::string &retErrorMessage) {
    ...
    std::string strMessage = service.ToString() + boost::lexical_cast<std::string>(masterNodeSignatureTime) + vchPubKey + vchPubKey2 + boost::lexical_cast<std::string>(PROTOCOL_VERSION) + HexStr(donationAddress) + boost::lexical_cast<std::string>(donationPercentage);
...
bool CActiveMasternode::GetVinFromOutput(COutput out, CTxIn& vin, CPubKey& pubkey, CKey& secretKey) {
    ...
    for (const std::shared_ptr<CWallet>& pwalletMain : GetWallets()) {
        LegacyScriptPubKeyMan* spk_man = pwalletMain->GetLegacyScriptPubKeyMan();
        if (!spk_man) {
            spk_man = pwalletMain->GetOrCreateLegacyScriptPubKeyMan();
        }
        if (!spk_man)
            return false;
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    for (const std::shared_ptr<CWallet>& pwalletMain : GetWallets()) {
        ...
        pwalletMain->AvailableCoins(*locked_chain, vCoins,/* fOnlySafe*/ true, /*coinControl*/ nullptr,
                                    /*nMinimumAmount */ 1, /*nMaximumAmount */MAX_MONEY,
                                     /*nMinimumSumAmount*/ MAX_MONEY,/* nMaximumCount*/ 0,
                                    /*isSearchingMasternode*/ true);
    }

    // Filter
    for(const COutput& out: vCoins)
    {
        if(out.tx->tx->vout[out.i].nValue == MASTERNODEAMOUNT*COIN) { //exactly
