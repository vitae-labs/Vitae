    ...
    platformStyle(_platformStyle)
{
    ...
    // Connect signals
    connect(ui->payAmount, &VitaeAmountField::valueChanged, this, &SendCoinsEntry::payAmountChanged);
...
bool SendCoinsEntry::validate(interfaces::Node& node, bool IsSwapTransaction)
{
    if (!model)
        return false;
    ...
    if (!model->validateAddress(ui->payTo->text()) && !IsSwapTransaction)
    {
        ui->payTo->setValid(false);
        retval = false;
    }

    if (!model->validateSolviAddress(ui->payTo->text()) && IsSwapTransaction)
    {
        ui->payTo->setValid(false);
        retval = false;
    }
...
SendCoinsRecipient SendCoinsEntry::getValue(bool IsSwapTransaction)
{
    ...
    recipient.fIsSwapTransaction = IsSwapTransaction;
