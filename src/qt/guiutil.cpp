...
#include <qt/vitaeaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Vitae address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new VitaeAddressEntryValidator(parent));
    widget->setCheckValidator(new VitaeAddressCheckValidator(parent));
}

bool parseVitaeURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no vitae: URI
    if(!uri.isValid() || uri.scheme() != QString("vitae"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!VitaeUnits::parse(VitaeUnits::VITAE, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseVitaeURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseVitaeURI(uriInstance, out);
}

QString formatVitaeURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("vitae:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(VitaeUnits::format(VitaeUnits::VITAE, info.amount, false, VitaeUnits::separatorNever));
        paramCount++;
    }
...
void openMNConffile()
{
    fs::path pathDebug = GetDataDir() / "masternode.conf";

    /* Open maternode.conf with the associated application */
    if (fs::exists(pathDebug))
        QDesktopServices::openUrl(QUrl::fromLocalFile(boostPathToQString(pathDebug)));
}
...
bool openVitaeConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", VITAE_CONF_FILENAME));
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Vitae (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Vitae (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("vitae-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=Vitae (%s)\n", chain);
