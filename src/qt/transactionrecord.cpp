...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const interfaces::WalletTx& wtx)
{
    ...
    if (wtx.is_coinstake)
    {
        ...
        CAmount nValueOut = 0;
        size_t vout_size = wtx.tx->vout.size();
        for(unsigned int i = 0; i < vout_size - 2; i++)
        {
        ...
        // master node
        {
            const CTxOut mn_out = wtx.tx->vout.back();
            isminetype mine = wtx.txout_is_mine[vout_size -1];

            if(mine) {
                TransactionRecord sub(hash, nTime);
                CTxDestination address;
                sub.idx = vout_size -1; // vout index
                sub.credit = mn_out.nValue;
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                if (wtx.txout_address_is_mine[vout_size -1])
                {
                    //
                    sub.type = TransactionRecord::MasternodeReward;
                    sub.address = EncodeDestination(wtx.txout_address[vout_size -1]);
                }
                parts.append(sub);
            }
        }
        ...
        // Fundamental coin
        {
            const CTxOut fn_out = wtx.tx->vout.rbegin()[1];
            isminetype mine = wtx.txout_is_mine[vout_size -2];

            if(mine) {
                TransactionRecord sub(hash, nTime);
                CTxDestination address;
                sub.idx = vout_size -2; // vout index
                sub.credit = fn_out.nValue;
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                if (wtx.txout_address_is_mine[vout_size -2])
                {
                    //
                    sub.type = TransactionRecord::FundamentalNodeReward;
                    sub.address = EncodeDestination(wtx.txout_address[vout_size -2]);
                }
                parts.append(sub);
            }
        }
