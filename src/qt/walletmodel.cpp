...
#include <base58.h> // for solviAddress validation
...
bool WalletModel::validateSolviAddress(const QString &address, std::vector<unsigned char>& vchAddressData)
{
    if (!DecodeBase58(address.toStdString(), vchAddressData, 32))
        return false; // TODO: logPrint

    return vchAddressData.size() == 32;
}

bool WalletModel::validateSolviAddress(const QString &address)
{
    std::vector<unsigned char>vchAddressData;
    if (!DecodeBase58(address.toStdString(), vchAddressData, 32))
        return false; // TODO: logPrint
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    if (isStakeOnlyUnlocked()) {
        return StakeOnlyUnlocked;
    }
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        if (rcp.fIsSwapTransaction)
            fIsSwapTransaction = true;
        std::vector<unsigned char> vchAddressData;
        {   // User-entered vitae address / amount:
            if(!fIsSwapTransaction && !validateAddress(rcp.address))
            {
                return InvalidAddress;
            }
            if (fIsSwapTransaction && !validateSolviAddress(rcp.address, vchAddressData)) {
                return InvalidAddress;
            }
            if(rcp.amount <= 0)
            {
            ...
            if (fIsSwapTransaction)
                scriptPubKey = CScript() << OP_RETURN << ParseHex("536f6c7669") << vchAddressData;
            else
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        {
            if (!rcp.message.isEmpty()) // Message from normal vitae:URI (vitae:123...?message=example)
...
WalletModel::EncryptionStatus WalletModel::getEncryptionStatus() const
{
    ...
    else if (m_wallet->fWalletUnlockStakeOnly())
    {
...
bool WalletModel::setWalletLocked(bool locked, const SecureString &passPhrase, bool stakeOnly)
{
    ...
    {
        // Lock
        m_wallet->unsetfWalletUnlockStakeOnly();
        return m_wallet->lock();
    }
    else
    {
        // Unlock
        return m_wallet->unlock(passPhrase, stakeOnly);
    }
}

bool WalletModel::isStakeOnlyUnlocked()
{
    return m_wallet->fWalletUnlockStakeOnly();
}
...
WalletModel::UnlockContext WalletModel::requestUnlock()
{
    ...
    if (!was_locked && fUnlockForStakingOnly) {
    ...
    return UnlockContext(this, valid, was_locked && !fUnlockForStakingOnly);
}
...
bool WalletModel::bumpFee(uint256 hash, uint256& new_hash)
{
    ...
    questionString.append(VitaeUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("Increase:"));
    questionString.append("</td><td>");
    questionString.append(VitaeUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("New fee:"));
    questionString.append("</td><td>");
    questionString.append(VitaeUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee));
