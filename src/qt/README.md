This directory contains the VitaeQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Vitae UI.
...
ClientModel is used by the main application `vitaegui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a vitae:... link from a browser).
...
* UI elements like VitaeAmountField, which inherit from QWidget.
* `vitaestrings.cpp`: automatically generated
* `vitaeunits.(h/cpp)`: VITAE / mVITAE / etc handling
* `callback.h`
...
* don't change `local/vitae_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "vitae-qt" under "Run", which is where you can also add command line arguments)
