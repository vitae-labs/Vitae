...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/vitae-dev/2018-November/016518.html
...
bool AcceptableInputs(CTxMemPool& pool, TxValidationState &state, const CTransactionRef& ptx, bool ignoreFees)
{
    // Check if inputs are not being spent and also it have enough coins
    Coin coin;
    ...
    CCoinsViewCache view(&viewDummy);
    {
        ...
        coin = view.AccessCoin(ptx->vin[0].prevout);

        view.SetBackend(viewDummy);


    }
    if (!coin.IsSpent() && coin.out.nValue >= MASTERNODEAMOUNT*COIN) {
        LogPrintf("AcceptableInputs: accepted input.\n");
        return true;
    } else {
        LogPrintf("AcceptableInputs: rejected input.\n");
    }
...
CAmount GetMasternodeReward(const CAmount nblockreward)
{
    // 40% for test
    return (nblockreward / 100) * 40;
}

CAmount GetFnReward(const CAmount nblockreward)
{
    // 40% for test
    return (nblockreward / 100) * 40;
}
...
{
    // mark inputs spent
    if (!(tx.IsCoinBase() || (tx.vin[0].scriptSig[0]==0xc2))) {
...
int GetInputAge(CTxIn& vin)
{
    ...
    CCoinsViewCache view(&viewDummy);
    {
        ...
        const Coin& coins_ = view.AccessCoin(vin.prevout);

        view.SetBackend(viewDummy);

        if (true ){ // todo check later
            if(coins_.nHeight < 0) return 0;
            return (::ChainActive().Tip()->nHeight+1) - coins_.nHeight;
        }
        else {
            return -1;
        }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // TODO: check for staking rewards
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight, txfee) && pindex->nHeight > FORK_HEIGHT) {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            TxValidationState tx_state;
            if (pindex->nHeight!=2665936 && pindex->nHeight > FORK_HEIGHT && fScriptChecks && !CheckInputScripts(tx, tx_state, view, flags, fCacheResults, fCacheResults, txdata[i], g_parallel_script_checks ? &vChecks : nullptr)) {
                // Any transaction validation failure in ConnectBlock is a block consensus failure
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                              tx_state.GetRejectReason(), tx_state.GetDebugMessage());
    ...
    if (block.IsProofOfStake() && stakereward > blockReward && pindex->nHeight > FORK_HEIGHT) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-stakebase-amount");
    }
    ...
    if (!control.Wait()) { //todo fix checkque failure on p2sh spends
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.vtx.size() < 2 && block.IsProofOfStake())
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-stake-missing", "block has less than 2 txn");
    ...
    if (block.nTime > FORK_TIME) {
        ...
        for (unsigned int i = 2; i < block.vtx.size(); i++)
            if (block.vtx[i]->IsCoinStake())
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-multiple", "more than one coinstake");

        if (block.IsProofOfStake() && block.vtx[0]->GetValueOut() != 0)
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-not-empty", "coinbase not empty for pos block");
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // added tolerance of 300 seconds
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast()-300)
 ...
 *  Note that -reindex-chainstate skips the validation that happens here!
 */
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, bool testblock = false)
{
    ...
    if (!pindexPrev)
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_PREV, "bad-pindex-prev", "current block is not genesis but has null previous");

    if (nHeight > FORK_HEIGHT && block.IsProofOfWork() && !testblock)
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-block", "proof-of-work block after pos start");

    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams) && nHeight > FORK_HEIGHT)
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-diffbits", block.IsProofOfStake() ? "incorrect proof of stake":"incorrect proof of work");
