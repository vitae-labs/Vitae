...
#include <masternodes/signhelper_mn.h>
 ...
 */
static const char* VITAE_PID_FILENAME = "vitaed.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", VITAE_PID_FILENAME)));
}
...
static boost::thread threadBitpool;
...
void Shutdown(NodeContext& node)
{
    ...
    threadBitpool.interrupt();
    if (threadBitpool.joinable())
        threadBitpool.join();
    ...
    DumpMasternodes(); // dump to masternodes.dat
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", VITAE_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", VITAE_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-staking", "Start staking after connection to atleast 3 nodes, requires full synced.", ArgsManager::ALLOW_ANY, OptionsCategory::STAKING);
    gArgs.AddArg("-forcestaking", "Force to stake without connection default is false", ArgsManager::ALLOW_ANY, OptionsCategory::STAKING);
    gArgs.AddArg("-masternode", "Start masternode after connection to atleast 3 nodes, requires full synced.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODES);
    gArgs.AddArg("-masternodeaddr", "Masternode address (a valid node address).", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODES);
    gArgs.AddArg("-masternodeprivkey", "Masternode private key to sign masternode messages.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODES);
    gArgs.AddArg("-sporkkey", "Spork key to activate spork.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODES);
    gArgs.AddArg("-masternodepaymentskey", "masternode payments key to ativate mn payment.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODES);
    gArgs.AddArg("-mnconflock", "masternodes from masternode.conf and lock outpoint, default is true.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODES);
    gArgs.AddArg("-mnconf", "masternodes conf file default is masternode.conf.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODES);
    //mnconf
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/vitae-labs/vitae>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i").translated, 2009, COPYRIGHT_YEAR) + " ") + "\n" +
           strprintf(_("Copyright (C) 2020 - 2021 The Vitae Core Developers").translated) + "\n"
           "\n" +
...
bool AppInitMain(NodeContext& node)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", VITAE_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if vitae is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if vitae is started while in a temporary directory.\n",
    ...
    g_rpc_node = &node;
    g_mn_node = &node; // todo : g_rpc_node = g_mn_node as ref
#if ENABLE_ZMQ
    ...
    // init masternode and spork key
    std::string mn_key = gArgs.GetArg("-masternodepaymentskey", "");
    std::string spork_key = gArgs.GetArg("-sporkkey", "");

    if (mn_key != "") {
        if (!masternodePayments.SetPrivKey(mn_key)){

            return InitError(_("Unable to sign masternode payment winner, wrong key?").translated);
        }
    }
    if (spork_key != "") {
        if (!sporkManager.SetPrivKey(spork_key)){
    ...
    if(!masternodeConfig.read(strErr)) {
        return InitError(_(strErr.c_str()).translated);
    }
    ...
    if(gArgs.GetBoolArg("-mnconflock", true)) {
        ...
        for (CMasternodeConfig::CMasternodeEntry mne : masternodeConfig.getEntries()) {
            ...
            for (const std::shared_ptr<CWallet>& pwalletMain : GetWallets()){LOCK(pwalletMain->cs_wallet); pwalletMain->LockCoin(outpoint);}// This is dirty fix,
    ...
    // start threadbitpool
    CConnman *pconnman = node.connman.get();
    /*boost::thread **/threadBitpool = boost::thread(&ThreadBitPool, pconnman);
    ...
    // ********************************************************* Step 14: run stakerthread
    bool fstaking = gArgs.GetBoolArg("-staking", true);
    fForceStaking = gArgs.GetBoolArg("-forcestaking", false);
    // TODO: staking inside wallet
    if (GetWallets()[0] && fstaking) { // todo: GetMainWallet()
        threadGroup.create_thread(std::bind(&StakeThreadMan, GetWallets()[0], node.connman.get(), node.mempool));
        //StakeThreadMan(threadGroup, GetWallets()[0], node.connman.get(), node.mempool, fShutdownRequested);
    }
